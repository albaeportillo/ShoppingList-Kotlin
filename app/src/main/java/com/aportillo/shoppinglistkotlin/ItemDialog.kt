...
class ItemDialog : DialogFragment(), AdapterView.OnItemSelectedListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        if (this.arguments != null) {
            val mArgs = this.arguments
             userId = EncodeString(mArgs!!.getString("userId").toString())
        }
    ...
    private fun setUpEditTextFields(builder: AlertDialog.Builder) {
        ...
        if (arguments != null && arguments.containsKey(ShoppinListActivity.KEY_ITEM_TO_EDIT)) {
            val shoppingListItem = arguments.getSerializable(ShoppinListActivity.KEY_ITEM_TO_EDIT) as Item
   ...
   //     myRef.setValue("Hello, World!")
    ...
    private fun handleItemEdit() {
        val itemToEdit = arguments?.getSerializable(ShoppinListActivity.KEY_ITEM_TO_EDIT) as Item

        var oldTotalCost = ShoppinListActivity.totalCost
        var oldItemCost = itemToEdit.price * itemToEdit.quantity.toFloat()

        editItem(itemToEdit)

        ShoppinListActivity.totalCost = (oldTotalCost - oldItemCost) + (itemToEdit.quantity.toFloat() * itemToEdit.price)
